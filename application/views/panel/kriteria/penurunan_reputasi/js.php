<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#tabel').DataTable({
            ...
            "ajax": {
                "url": '<?= base_url('kriteria/penurunan_reputasi/dat_list') ?>',
                "type": "POST",
    ...
    async function edit(id) {
        ...
        await $.ajax({
            url: '<?= base_url('kriteria/penurunan_reputasi/edit/') ?>' + id,
            type: "GET",
            async: false,
            dataType: "JSON",
            success: function(data) {
                ...
                $('[name="penurunan_reputasi"]').val(data.penurunan_reputasi);
                $('[name="level_dampak"]').val(data.level_dampak);

                $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
                $('.modal-title').text('Edit Kriteria Penurunan Reputasi'); // Set title to Bootstrap modal title
            },
    ...
    function save() {
        ...
        var url;
        url = '<?= base_url('kriteria/penurunan_reputasi/save') ?>';
    ...
    function _delete(id, nama) {
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: '<?= base_url('kriteria/penurunan_reputasi/delete/') ?>' + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function(data) {
