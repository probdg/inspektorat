<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#tabel').DataTable({
            ...
            "ajax": {
                "url": '<?= base_url('kriteria/penurunan_kinerja/dat_list') ?>',
                "type": "POST",
    ...
    async function edit(id) {
        ...
        await $.ajax({
            url: '<?= base_url('kriteria/penurunan_kinerja/edit/') ?>' + id,
            type: "GET",
            async: false,
            dataType: "JSON",
            success: function(data) {
                ...
                $('[name="penurunan_kinerja"]').val(data.penurunan_kinerja);
                $('[name="level_dampak"]').val(data.level_dampak);

                $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
                $('.modal-title').text('Edit Kriteria Penurunan Kinerja'); // Set title to Bootstrap modal title
            },
    ...
    function save() {
        ...
        var url;
        url = '<?= base_url('kriteria/penurunan_kinerja/save') ?>';
    ...
    function _delete(id, nama) {
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: '<?= base_url('kriteria/penurunan_kinerja/delete/') ?>' + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function(data) {
