<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#tabel').DataTable({
            ...
            "ajax": {
                "url": '<?= base_url('kriteria/kemungkinan/dat_list') ?>',
                "type": "POST",
    ...
    async function edit(id) {
        ...
        await $.ajax({
            url: '<?= base_url('kriteria/kemungkinan/edit/') ?>' + id,
            type: "GET",
            async: false,
            dataType: "JSON",
            success: function(data) {
                $('[name="id"]').val(data.id);
                $('[name="frekuensi"]').val(data.frekuensi);
                $('[name="reg"]').val(data.reg);
                $('[name="probabilitas"]').val(data.probabilitas);
                $('[name="level"]').val(data.level);

                $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
                $('.modal-title').text('Edit Kriteria Kemungkinan'); // Set title to Bootstrap modal title
            },
    ...
    function save() {
        ...
        var url;
        url = '<?= base_url('kriteria/kemungkinan/save') ?>';
    ...
    function _delete(id, nama) {
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: '<?= base_url('kriteria/kemungkinan/delete/') ?>' + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function(data) {
