...
class Form3b extends CI_Controller
{
    ...
    public function dat_list()
    {
        ...
        foreach ($list as $list) {
            $edit   = '<a class="btn btn-primary btn-icon btn-sm" href="javascript:void(0)"  title="Edit" onclick="edit_risk_opd(' . "'" . $list->id . "'" . ')"><i class="fa fa-edit"></i></a>';
            $delete = ' <a class="btn btn-danger btn-icon btn-sm" href="javascript:void(0)"  title="Hapus" onclick="_delete_risk_opd(' . "'" . $list->id . "'" . ',' . "'" . $list->kode_risiko . "'"  . ')"><i class="fa fa-trash"></i></a>';
            $no++;
            $row = array();
            $row[] = $no;
    ...
    {
        $delete = $this->db->delete('f3b_risk_opd', ['id' => $id]);
        if ($delete) {
    ...
    public function save()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $kode = 'RSO.' . substr($tahun, 2, 4) . '.' . $kode_jenis . '.' . $kode_opd . '.' . str_pad($no_urut, 2, '0', STR_PAD_LEFT);
            ...
            if (!empty($id)) {
                ...
                $this->db->insert('f3b_risk_opd', $data);
    ...
    public function export()
    {
        ...
        $IdentRisk = $this->db->get_where('f3b_risk_opd', ['tahun' => $tahun, 'opd_id' => $opd_id, 'urusan' => $urusan, 'id_rpjmd' => $id_rpjmd])->result_array();
        ...
        $this->load->view('form/export3b', $data);
    }
}
