...
class sasaran extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data = [
            ...
            'subtitle' => 'Sasaran Strategis Pemda',
            'sess_opd'  => $this->session->userdata('opd'),
            'namaPemda' => $namaPemda,
            'idPemda'   => $idPemda,
            'content'   => 'panel/sasaran/index',
            'js'        => 'panel/sasaran/js',
    ...
    public function dat_list()
    {
        ...
        foreach ($list as $list) {
            ...
            $delete = ' <a class="btn btn-danger btn-icon btn-sm" href="javascript:void(0)"  title="Hapus" onclick="_delete(' . "'" . $list->id . "'" . ',' . "'" . substr($list->sasaran, 0, 10) . "'"  . ')"><i class="fa fa-trash"></i></a>';
    ...
    public function save()
    {
        ...
        $config = [
            [
                ...
                'rules' => 'required',
                'errors' => [
                    'required' => 'Masukan sasaran Strategis',
                ],
            ],
            [
                'field' => 'rpjmd',
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($id) {
                $this->db->update('ref_sasaran_strategis', $data, ['id' => $id]);
                $msg = 'Data berhasil diperbaharui';
            } else {
                $this->db->insert('ref_sasaran_strategis', $data);
    ...
    {
        $row = $this->db->get_where('ref_sasaran_strategis', ['id' => $id])->row();
        echo json_encode($row);
    }

    public function delete($id)
    {
        $delete = $this->db->delete('ref_sasaran_strategis', ['id' => $id]);
        if ($delete) {
