...
class Misi extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data = [
            ...
            'subtitle' => 'Misi Strategis Pemda',
            'sess_opd'  => $this->session->userdata('opd'),
            'namaPemda' => $namaPemda,
            'idPemda'   => $idPemda,
            'content'   => 'panel/misi/index',
            'js'        => 'panel/misi/js',
    ...
    {
        $tabel = 'ref_misi_strategis';
        $column_order = array(null, 'misi');
        $coloumn_search = array('misi');
        ...
        foreach ($list as $list) {
            ...
            $delete = ' <a class="btn btn-danger btn-icon btn-sm" href="javascript:void(0)"  title="Hapus" onclick="_delete(' . "'" . $list->id . "'" . ',' . "'" . substr($list->misi, 0, 10) . "'"  . ')"><i class="fa fa-trash"></i></a>';
    ...
    public function save()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($id) {
                $this->db->update('ref_misi_strategis', $data, ['id' => $id]);
                $msg = 'Data berhasil diperbaharui';
            } else {
                $this->db->insert('ref_misi_strategis', $data);
    ...
    {
        $row = $this->db->get_where('ref_misi_strategis', ['id' => $id])->row();
        echo json_encode($row);
    }

    public function delete($id)
    {
        $delete = $this->db->delete('ref_misi_strategis', ['id' => $id]);
        if ($delete) {
