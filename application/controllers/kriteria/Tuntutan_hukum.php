...
class Tuntutan_hukum extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data = [
            ...
            'subtitle' => 'Kriteria Tuntutan Hukum',
            'sess_opd'  => $this->session->userdata('opd'),
            'namaPemda' => $namaPemda,
            'idPemda'   => $idPemda,
            'content'   => 'panel/kriteria/tuntutan_hukum/index',
            'js'        => 'panel/kriteria/tuntutan_hukum/js',
    ...
    {
        $tabel = 'ref_tuntutan_hukum';
        $column_order = array(null, 'tuntutan_hukum');
        $coloumn_search = array('tuntutan_hukum');
    ...
    {
        $tuntutan_hukum = $this->input->post('tuntutan_hukum', TRUE);
        ...
        $config = [
            [
                'field' => 'tuntutan_hukum',
                'rules' => 'required',
                'errors' => [
                    'required' => 'Masukan Kriteria Tuntutan Hukum',
                ],
            ],
            [
                'field' => 'level_dampak',
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($id) {
                $this->db->update('ref_tuntutan_hukum', $data, ['id' => $id]);
                $msg = 'Data berhasil diperbaharui';
            } else {
                $this->db->insert('ref_tuntutan_hukum', $data);
    ...
    {
        $row = $this->db->get_where('ref_tuntutan_hukum', ['id' => $id])->row();
        echo json_encode($row);
    }

    public function delete($id)
    {
        $delete = $this->db->delete('ref_tuntutan_hukum', ['id' => $id]);
        if ($delete) {
