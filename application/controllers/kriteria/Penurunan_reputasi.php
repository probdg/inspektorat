...
class Penurunan_reputasi extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data = [
            ...
            'subtitle' => 'Kriteria Penurunan Reputasi',
            'sess_opd'  => $this->session->userdata('opd'),
            'namaPemda' => $namaPemda,
            'idPemda'   => $idPemda,
            'content'   => 'panel/kriteria/penurunan_reputasi/index',
            'js'        => 'panel/kriteria/penurunan_reputasi/js',
    ...
    {
        $tabel = 'ref_penurunan_reputasi';
        $column_order = array(null, 'penurunan_reputasi');
        $coloumn_search = array('penurunan_reputasi');
    ...
    {
        $penurunan_reputasi = $this->input->post('penurunan_reputasi', TRUE);
        ...
        $config = [
            [
                'field' => 'penurunan_reputasi',
                'rules' => 'required',
                'errors' => [
                    'required' => 'Masukan Kriteria Penurunan Reputasi',
                ],
            ],
            [
                'field' => 'level_dampak',
        ...
        if ($this->form_validation->run() == TRUE) {
            $data = [
                'penurunan_reputasi'      => $penurunan_reputasi,
            ...
            if ($id) {
                $this->db->update('ref_penurunan_reputasi', $data, ['id' => $id]);
                $msg = 'Data berhasil diperbaharui';
            } else {
                $this->db->insert('ref_penurunan_reputasi', $data);
    ...
    {
        $row = $this->db->get_where('ref_penurunan_reputasi', ['id' => $id])->row();
        echo json_encode($row);
    }

    public function delete($id)
    {
        $delete = $this->db->delete('ref_penurunan_reputasi', ['id' => $id]);
        if ($delete) {
